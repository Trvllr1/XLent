...
class TestStore {
  private stmts = {
    ...
    updateAssertion: db.prepare('UPDATE model_tests SET assertion = ? WHERE id = ? AND model_id = ?'),
  };
  ...
  updateAssertion(modelId: string, testId: string, assertion: object): boolean {
    return this.stmts.updateAssertion.run(JSON.stringify(assertion), testId, modelId).changes === 1;
  }
}
