...
describe('previewMutation', () => {
  ...
  it('selects affected-graph tests while retaining the full suite as the commit gate', () => {
    const { model, workbook } = buildFixture();
    const preview = previewMutation(model, workbook, {
      actor: { id: 'user-1', type: 'human' },
      rationale: 'Review only tests connected to the changed output',
      operations: [{ type: 'setParameterValue', parameterId: 'revenue', value: 25 }],
    }, [{
      id: 'affected-test',
      modelId: model.id,
      name: 'Double revenue remains correct',
      category: 'business',
      assertion: { type: 'equals', left: 'double-revenue', right: 50 },
      autoGenerated: false,
    }, {
      id: 'unrelated-test',
      modelId: model.id,
      name: 'Unrelated structural inventory',
      category: 'structural',
      assertion: { type: 'is_numeric', left: '__meta_formula_count' },
      autoGenerated: true,
    }]);

    expect(preview.relevantTestIds).toEqual(['affected-test']);
    expect(preview.relevantTestResults).toEqual([expect.objectContaining({ testId: 'affected-test', status: 'pass' })]);
    expect(preview.testResults).toHaveLength(2);
  });
